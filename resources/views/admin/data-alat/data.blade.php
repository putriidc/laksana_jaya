@extends('kepala-gudang.layout') @section('content')
    <div>
        ...
        <div class="flex justify-between items-center mb-6 max-[600px]:flex-col max-[600px]:gap-y-2 max-[600px]:items-start">
            <a href="{{ route('alats.create') }}" class="block px-4 py-2 border-2 border-[#9A9A9A] rounded-lg">Input Alat
                +</a>
            <form method="GET" action="{{ route('alats.index') }}" class="flex items-center gap-x-2">
        ...
        <section class="flex flex-wrap gap-2">
            @foreach ($alats as $item)
                <div
                    ...
                    <a href="{{ route('alats.show', $item->id) }}"
