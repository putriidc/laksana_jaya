@extends('admin.layout') @section('content')
    <div>
        ...
        <div class="flex justify-between items-center mb-6 max-[600px]:flex-col max-[600px]:gap-y-2 max-[600px]:items-start">
            <a href="{{ route('alatsAdmin.create') }}" class="block px-4 py-2 border-2 border-[#9A9A9A] rounded-lg">Input Alat
                +</a>
            <form method="GET" action="{{ route('alatsAdmin.index') }}" class="flex items-center gap-x-2">
                ...
                <a target="_blank" href="{{ route('alat-admin.print') }}" class="border-[#9A9A9A] border-2 rounded-lg py-[10px] px-[10px] bg-white cursor-pointer">
        ...
        <section class="flex flex-wrap gap-2">
            @foreach ($alats as $item)
                <div
                    ...
                    <a href="{{ route('alatsAdmin.show', $item->id) }}"
