...
@section('content')
    <div class="flex flex-col items-center">
        <h1 class="text-2xl font-bold mb-6 w-full">Edit Peminjaman</h1>
        <div class="shadow-[0px_0px_15px_rgba(0,0,0,0.25)] w-full p-10 rounded-lg bg-white">
            <form method="POST" action="{{ route('alat-pinjam-admin.update', $alatPinjam->id) }}" class="flex flex-col gap-y-4">
                @csrf
                @method('PUT')
                <div class="flex items-center max-[600px]:flex-col max-[600px]:items-start max-[600px]:gap-y-2">
                    ...
                    <input type="date" name="tanggal" id="" value="{{ $alatPinjam->tanggal }}"
                ...
                <div class="flex items-center max-[600px]:flex-col max-[600px]:items-start max-[600px]:gap-y-2">
                    ...
                    <input type="hidden" name="kode_alat" value="{{ $alatPinjam->kode_alat }}">
                </div>
                ...
                <div class="flex items-center max-[600px]:flex-col max-[600px]:gap-y-2 max-[600px]:items-start">
                    ...
                    <select name="kode_akun" id="kode_akun"
                        ...
                        <option value="{{ $alatPinjam->kode_akun }}" selected>
                            {{ $proyekDipinjam->nama_proyek }}
                        </option>
                ...
                <div class="flex items-center max-[600px]:flex-col max-[600px]:items-start max-[600px]:gap-y-2">
                    ...
                    <input type="text" id="pic" value="{{ $proyekDipinjam->pic }}"
                        class="w-full outline-none bg-[#D9D9D9]/40 rounded-sm px-4 py-2 " readonly>
                </div>
                <div class="flex items-center max-[600px]:flex-col max-[600px]:items-start max-[600px]:gap-y-2">
                    <label class="w-[180px] font-medium">Keterangan</label>
                    <input type="text" name="keterangan" value="{{ $alatPinjam->keterangan }}"
                        class="w-full outline-none bg-[#D9D9D9]/40 rounded-sm px-4 py-2">
                </div>
                <div class="flex items-center max-[600px]:flex-col max-[600px]:items-start max-[600px]:gap-y-2">
                    <label class="w-[180px] font-medium">Jumlah Pinjam</label>
                    <input type="number" name="qty" value="{{ $alatPinjam->qty }}"
                        class="w-full outline-none bg-[#D9D9D9]/40 rounded-sm px-4 py-2">
                </div>
                <div class="flex mt-4">
